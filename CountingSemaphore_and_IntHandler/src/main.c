...
int main(void)
{
	...
	xSemaphoreCount = xSemaphoreCreateCounting(10,0);

	if(xSemaphoreCount!=NULL)
	{
...
void vTask1(void *params)
{
	while(1)
	{
		xSemaphoreTake(xSemaphoreCount, portMAX_DELAY );
		{
...
static void prvSetupUSART(void)
{
	...
	//interrupt config for the button B1 which is PA0
...
void EXTI0_IRQHandler  ()
{
	...
	sprintf(msg,"hello button\n");

	xSemaphoreGiveFromISR(xSemaphoreCount,&xHigherPriorityTaskWoken);
	xSemaphoreGiveFromISR(xSemaphoreCount,&xHigherPriorityTaskWoken);
	xSemaphoreGiveFromISR(xSemaphoreCount,&xHigherPriorityTaskWoken);
	xSemaphoreGiveFromISR(xSemaphoreCount,&xHigherPriorityTaskWoken);
